...
// Computation matrices structure
...
void saveModelToFile(const HMMParameters &model, int digit) {
    ...
    filepath << "models/model_" << digit << ".txt";
    ofstream file(filepath.str());
    
    if (!file) {
        cerr << "Error: Could not save model for digit " << digit << "\n";
        return;
    }
...
bool loadModelFromFile(HMMParameters &model, int digit) {
    ...
    if (!file) {
        cerr << "Error: Could not load model for digit " << digit << "\n";
        return false;
    }
                ...
                for (int j = 0; j < STATE_COUNT; j++) {
                    if (denominator > 0) {
                        compute.transitionProbs[t][i][j] = (compute.forwardProbs[t][i] * 
                                                          model.transitionMatrix[i][j] * 
                                                          model.emissionMatrix[j][compute.observationSeq[t+1]] * 
                                                          compute.backwardProbs[t+1][j]) / denominator;
                    } else {
            ...
            if (finalDenominator > 0) {
                compute.stateProbs[compute.seqLength-1][i] = 
                    (compute.forwardProbs[compute.seqLength-1][i] * 
                     compute.backwardProbs[compute.seqLength-1][i]) / finalDenominator;
            } else {
...
// Training function
void trainModels(vector<HMMParameters> &digitModels) {
    cout << "Starting model training for all digits...\n";

    for (int digit = 0; digit < DIGIT_COUNT; digit++) {
        cout << "\nTraining digit " << digit << "...\n";
        initializeModelFromFiles(digitModels[digit]);
        ...
        for (int iteration = 0; iteration < ITERATION_COUNT; iteration++) {
            ...
            for (int sample = 1; sample <= TRAINING_SET_SIZE; sample++) {
                ostringstream filepath;
                filepath << DATA_DIRECTORY << "210101070_E_" << digit << "_" << sample << ".txt";
                
                HMMParameters currentModel = digitModels[digit];
                baumWelch(currentModel, filepath.str());
                iterationModels.push_back(currentModel);
            }

            if (!iterationModels.empty()) {
                averageModels(iterationModels, digitModels[digit]);
            }
        }
        
        cout << "Saving model for digit " << digit << "...\n";
        saveModelToFile(digitModels[digit], digit);
    }
...
// Testing function
void testModels(const vector<HMMParameters> &digitModels) {
    int correctCount = 0;
    int uncertainCount = 0;
    int totalTests = DIGIT_COUNT * TEST_SET_SIZE;

    cout << "\nStarting model testing phase...\n";

    for (int actualDigit = 0; actualDigit < DIGIT_COUNT; actualDigit++) {
        int digitCorrect = 0;
        int digitUncertain = 0;
        
        cout << "\nTesting digit " << actualDigit << "...\n";

        for (int sample = 1; sample <= TEST_SET_SIZE; sample++) {
            ostringstream filepath;
            filepath << DATA_DIRECTORY << "210101070_E_" << actualDigit << "_" 
                    << (TRAINING_SET_SIZE + sample) << ".txt";

            // Get probabilities for all models
            vector<pair<int, long double>> modelProbabilities;
            for (int modelDigit = 0; modelDigit < DIGIT_COUNT; modelDigit++) {
                long double probability = evaluateSequence(digitModels[modelDigit], filepath.str());
                modelProbabilities.push_back(make_pair(modelDigit, probability));
            }

            // Find the best model
            auto bestModel = max_element(modelProbabilities.begin(), modelProbabilities.end(),
                [](const pair<int, long double> &a, const pair<int, long double> &b) { return a.second < b.second; });

            int predictedDigit = bestModel->first;
            ...
            if (isUncertain) {
                digitUncertain++;
                uncertainCount++;
                cout << "Sample " << sample << " - Actual: " << actualDigit
                     << " - Result: Uncertain (between " << predictedDigit
        ...
        if (TEST_SET_SIZE - digitUncertain > 0) {
            digitAccuracy = (static_cast<double>(digitCorrect) / 
                           (TEST_SET_SIZE - digitUncertain)) * 100.0;
        }

        cout << "\nDigit " << actualDigit << " Statistics:\n"
             << "Correct: " << digitCorrect << "\n"
             << "Uncertain: " << digitUncertain << "\n"
    ...
    if (totalTests - uncertainCount > 0) {
        overallAccuracy = (static_cast<double>(correctCount) / 
                          (totalTests - uncertainCount)) * 100.0;
    }
...
void evaluateLiveRecording(const vector<HMMParameters> &digitModels) {
    ...
    vector<pair<int, long double>> modelProbabilities;
    for (int modelDigit = 0; modelDigit < DIGIT_COUNT; modelDigit++) {
        long double probability = evaluateSequence(digitModels[modelDigit], 
    ...
    cout << "\nRecognition Result:\n";
    cout << "Detected Digit: " << bestModel->first << "\n";
}

// Main function
int main() {
    vector<HMMParameters> digitModels(DIGIT_COUNT);
    bool isModelTrained = false;
    
    while (true) {
        cout << "\nHidden Markov Model Digit Recognition System\n"
             << "----------------------------------------\n"
             << "1. Train New Models\n"
        ...
        switch (choice) {
            ...
            case '2':
                ...
                isModelTrained = true;
                for (int digit = 0; digit < DIGIT_COUNT; digit++) {
                    if (!loadModelFromFile(digitModels[digit], digit)) {
            ...
            case '4':
                if (!isModelTrained) {
                    ...
                    evaluateLiveRecording(digitModels);
                }
                break;
